...
%
In practice, DSM typically predicts at either the top or centre of the specified depth slices, as a pointwise prediction. This can be based on the property in the training data at that point location or integrated over the depth slice, typically by smoothing splines over the properties measured from bulk samples of genetic horizons, which do not correspond to the fixed depth slices.


\par
So the correspondence with vertical ``reality'' is reduced to accuracy of either the value at the depth slice top or centre point, or its mean over the depth slice.
%
...
%
An example of the first case is particle-size class in an alluvial landscape, of the second case the organic matter profile in Podzols. 
...
%
Other boundaries are not obviously linked to the landscape, but may have been inferred from depositional history discovered in the land user's preferences for fields. 
%
...
\par
DSM predicts over grid cells much larger than a pedon, at finest $10 \ctimes 10 \; \mathrm{m}$.
%
...
\par
These issues are discussed in detail by \citet{maloneSpatialScalingDigital2013}.
...
%
\par
Cartographic resolution in DSM is expressed as the grid cell size.
%
...
\par
The link between the two concepts of resolution is that the more general the cartography, the more general the category. USDA Soil Taxonomy was explicitly designed from the bottom up to group soil series in increasingly-general classes, about which increasingly-general statements can be made, and which it was expected that increasingly small-scale maps would be needed through the progression.
%
This latter was not realistic in some landscapes \citep{EdmondsStatisticalevaluationtaxonomic1986} but if only dominant classes are considered, over much of the soilscape this relation holds.
%
...
iSDA maps vs.\ colonial maps.

\begin{figure}
  \centering
  \includegraphics[width=0.7\textwidth]{./EKonyangoPart1.jpg}
  \caption{Soil class map, East Konyango district, Kenya. Source: \citep{KenyaDepartmentofAgricultureSoilsurveyEast1961}}
  \label{ekonya1}
\end{figure}
