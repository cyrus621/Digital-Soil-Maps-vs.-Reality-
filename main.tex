...
%
Figure \ref{fig:coahoma_clay} shows this for the original gSSURGO polygons and for four resolutions rasterized from this by the \texttt{terra::rasterize} R function, computed as values of the vector map under the centre point of the  pixel.
%
This approximates the usual DSM procedure of predicting a pixel by its centre point.

\begin{figure}
  \includegraphics[width=0.32\textwidth]{gssurgo-vectors.pdf}
  \hfill
 ...
 \caption{Clay concentration \%, Coahoma County (MS). Top: gSSURGO vectors (left); 10~m and 30~m rasterization (right); Bottom: Left to right: rasterizations at 100~m, 250~m, 1~km}
